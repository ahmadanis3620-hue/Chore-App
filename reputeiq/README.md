...
1. Set the root directory to `reputeiq/` — this is a standalone app inside the
   repository.
2. Set every required environment variable.
3. Build command is `npm run build` (runs `prisma generate` first).
4. Run `npm run db:deploy` against the production database on release.
5. Use the pooled connection string for `DATABASE_URL` and the direct one for
   `DIRECT_URL`.
6. `vercel.json` registers the cron schedules; set `CRON_SECRET` in the project
