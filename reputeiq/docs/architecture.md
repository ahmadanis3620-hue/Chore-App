...
> Status: MVP. This document describes what is actually implemented in `reputeiq/`,
> and flags anything that is deliberately stubbed.

---

## 1. Where this lives

ReputeIQ is a **standalone Next.js application** in the `reputeiq/` directory of this
repository. It does not share code, dependencies, or a database with the existing
NaborTask app at the repository root — that app is untouched.

```
Chore-App/
├── app/, lib/          # NaborTask (pre-existing, untouched)
└── reputeiq/           # ReputeIQ — this product
    ├── prisma/         # schema + migrations
    ├── scripts/        # seed + job runner CLIs
    ├── src/
